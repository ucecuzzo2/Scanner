...
// Done by: ** Gerald, Chris, Brian
void syntaxerror2(string lexeme,
                  tokentype token) { // come back later try string if wrong
...
void tense() {
  ...
  switch (next_token()) { // verbpast, verbpasteng, verb, verbneg, else call
                          // syntax error 2
  case VERBPAST:
    match(VERBPAST);
    break;
...
void afterObject() {
  ...
  switch (next_token()) { // if verb, word1, word2, pronoun, else call syntax
                          // error 2
  case VERB:
    verb();
    getEword();
...
ofstream outputfile("translated.txt"); // opening translated.txt file to write
                                       // on

void gen() {
  // cout << "Starting gen" << endl;
  switch (saved_token) { // Find token and output correct output to file
  case CONNECTOR:
  ...
  case IS:
    ...
    outputfile << "TENSE: " << tokenName[saved_token] << endl
               << endl; // Use array of tokens to output the correct token name.
    break;
  case WAS:
    outputfile << "DESCRIPTION: " << saved_E_word << endl;
    outputfile << "TENSE: " << tokenName[saved_token] << endl << endl;
    break;
  case WORD2:
...
int main() {
  ...
  for (int i = 0; !fin.eof(); i++) { // till end of file read in words
    ...
    words translation = {read_japanese,
                         read_english}; // make struct to save read in words
    dictionary.push_back(
        translation); // push the struct to our vector dictionary
  }
  //** closes lexicon.txt
  fin.close();

  // testing out our read in words
  /*for (vector<words>::iterator i = dictionary.begin(); i != dictionary.end();
  i++) { cout << i->japanese << "  " << i->english << endl;
  }*/
